          ...
          {servicesMap.map((service, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* Overlays */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent transition-opacity duration-300 group-hover:opacity-70" />
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-end">
                  <div className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center mb-4 text-white group-hover:bg-secondary group-hover:text-primary transition-colors duration-300">
                    <Icon className="w-6 h-6" />
                  </div>
                  
                  <h3 className="text-xl font-bold text-white mb-2 translate-y-0 group-hover:-translate-y-2 transition-transform duration-300">
                    {t(`services.items.${service.id}.name`)}
                  </h3>
                  
                  <p className="text-white/80 text-sm mb-4 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:-translate-y-2 transition-all duration-300 line-clamp-2">
                    {t(`services.items.${service.id}.description`)}
                  </p>

                  <Button 
                    variant="outline" 
                    className="w-full bg-transparent border-white text-white hover:bg-white hover:text-primary opacity-0 group-hover:opacity-100 translate-y-4 group-hover:-translate-y-2 transition-all duration-300"
                    onClick={() => {
                      const el = document.getElementById('enquiry');
                      if (el) el.scrollIntoView({ behavior: 'smooth' });
                      // In a real app we might pre-fill the form here using global state
                    }}
                  >
